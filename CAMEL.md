...
.marshal().json()
//.to("log:info")
.to("kamelet:kafka-sink-scram?bootstrapServers=warehouse-paris-kafka-bootstrap:9092&user=camel&password=s3cr3t&topic=warehouse-in")
```

```sh
kamel install
kamel run mqtt-kafka.groovy
```
